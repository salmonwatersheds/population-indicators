...
The goal of the script is to fix issues in NuSEDS and to merge the two constituent datasets all_areas_nuseds and conservation_unit_system_sites.
...
In the case `i =` `r i` above with series `r paste(iid,gfeid,sep = " - ")` was not merged to any of the alternative series because of its unknown run timing (see below). It is consequently removed from **NUSEDS**.
...
In the case `i =` `r i` above with series `r paste(iid,gfeid,sep = " - ")` was initially going to be merged to the alternative series with which is was 100% compatible. But is was decided to remove it from **NUSEDS** because its run timing is unknown.
...
In the case `i =` `r i` above with series `r paste(iid,gfeid,sep = " - ")` was not merged to the alternative series because of different run timings:

```{r, echo=F}
show
```

It was decided to associate the population `r iid` to the CU *East Vancouver Island-Georgia Strait (Summer 4-1)* in the PSE because of the same run timing and the fact that the Chemainus river is connected to this CU. The corresponding `CU_NAME` in **CUSS** is:
...
```
The reference of the `r paste(iid,gfeid,sep = " - ")` time series is added to **CUSS**. The CU-related fields in **CUSS** are filled with the values corresponding to the to the above `CU_NAME`. Here below is the row added to **CUSS**:
...
In the case `i =` `r i` above with series `r paste(iid,gfeid,sep = " - ")` can be merged to the alternative series `r paste(iid_alter,gfeid,sep = " - ")` with which it is 100% compatible. The run timing indicated in `POPULATION` in **NUSEDS** are not incompatible: 
...
However, we decide to merge the alternative time series to the focal one (i.e., change `r paste(iid,iid_alter,sep = " - ")` to `r paste(iid,gfeid,sep = " - ")`) instead of the contrary because the latter is longer and more recent. Consequently both **NUSEDS** and **CUSS** are edited.
...
Here the focal time series (i.e., a unique `IndexId` - `GFE_ID` not in **CUSS**) are compared to alternative series with a different `GFE_ID` and potentially a different `IndexId` (but of the same species). 
...
Notice in the example above that several time series have the same `IndexId`. Again this is problematic because normally only one `GFE_ID` can be associated to a given `IndexId` (i.e., `POP_ID`). We show here after the focal time series grouped with their alternative series and provide in each case the decision to make.

```{r, echo = F, fig.width = 17, fig.height = 13}
...
In case i = `r i`: series `r paste(iid,gfeid,sep = " - ")` is not merged to alternative series `r paste(iid,gfeid_a,sep = " - ")` because these chum salmon would not be able to make it up the fall. Consequently, the reference of the focal series is added to **CUSS**. Coordinates for this GFE_ID are added manually using Google Maps and point to just above Stamp falls, on the left of the Stamp river fish ladder.
...
# And the GFE_ID is not in the DFO stream file:
cond <- DFO_All_Streams_Segments$ID == as.numeric(gfeid)
DFO_All_Streams_Segments[cond,]
```
As seen above, we need to attribute coordinates to this new location because there is not any in the DFO stream file. We used Google Maps and selected the location just above Stamp Falls, on the left of the Stamp river fish ladder:
...
In case i = `r i`: the focal series `r paste("CN_47277","2464",sep = " - ")` in blue is not merged to the alternative series `r paste("CN_47277","142",sep = " - ")` in red ; it is instead added to **CUSS**. The coordinate for this location had to be defined manually using Google Maps; they corresponds to the mouth of Cottonwood river:
...
In case i = `r i`: all five focal series are added to **CUSS**. Their coordinates were unknown so we defined them manually using Google Maps and the map in page 31 of [Arbeider et al. 2020. Interior Fraser Coho Salmon Recovery Potential Assessment](https://waves-vagues.dfo-mpo.gc.ca/library-bibliotheque/40888721.pdf):
...
In case i = `r i`: the focal series `r paste("CO_46795","33103",sep = " - ")` in blue is merged to the alternative series `r paste("CO_46795","281",sep = " - ")` in red. The other focal series `r paste("CO_46795","1921661712",sep = " - ")` in green is added to **CUSS**. Its coordinates have to be defined manually:
...
In the remaining cases (i = `r paste0(is,collapse = ", ")`): the focal series have the same `IndexId` (i.e. `POP_ID`) but a different `GFE_ID` as their alternative series: the location CLEAR WATER CREEK is not present in **CUSS** and the location PHILLIPS RIVER is proposed as an alternative. We do not merge the focal series to their alternative series and add to **CUSS** instead.
...
There remain `r nb_rows` series in **NuSEDS** with no alternative series (see below). These series are removed because we cannot associate them to a `CU_NAME`. It future, it would be worth attempting to salvage the longest series in the list.
...
for(r in 1:nrow(trackRecord_nuseds_nocuss)){
  ...
  legend("topright",c("WATERBODY:",WATERBODY),bty = "n")
...
So far we have not been able to determine which 0s are actual 0s and those which should be NAs. We consequently replace all 0s by NAs (`r n = nb cases`, or `r paste0(round(prop_cases,2),"%)` of the data points).

## Fix duplicated series and points

There are many cases where a CU in a given location (i.e., a unique `GFE_ID`) has multiple series (i.e. `IndexId`/`POP_ID`) with clear duplicates or single data point that are not worth keeping. To fix these issues we proceed as follow:
- Case 1: one of the duplicated series has only one data point:
  - if it is complementary: merge to the other (longer) series
  - if it is in conflict or duplicate: remove the focal series
...
# write.csv(added_all,paste0(wd_data,"/added_all_TEMP.csv"), row.names = F)
# 
# read.csv(paste0(wd_data,"/all_areas_nuseds_TEMP.csv"), header = T)
# read.csv(paste0(wd_data,"/conservation_unit_system_sites_TEMP.csv"), header = T)
# read.csv(paste0(wd_data,"/removed_all_TEMP.csv"), header = T)
# read.csv(paste0(wd_data,"/added_all_TEMP.csv"), header = T)
```
