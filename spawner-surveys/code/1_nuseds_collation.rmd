...
```{r, include=FALSE}
#'* Add the field StatArea to NUSEDS * BRUNO TO DELTE
...
The goal is to remove these `IndexId` & `GFE_ID` references from **CUSS** and to specify if they were simply not present in the orginal **NUSEDS** or only contained NAs and/or 0s.
...
In case i = `r i`: the focal series `r paste("CO_46170","2461",sep = " - ")` in green is merged to the alternative series `r paste("CO_46170","213",sep = " - ")` in red.
...
The other focal series `r paste("CO_46170","54282",sep = " - ")` in blue is instead added to **CUSS**. Its `GFE_ID` is not in **CUSS**: but it is in **DFO streams file** but without coordinates:
...
```{r}
cond <- DFO_hatchery$STOCK_GFE_ID == 54282 & !is.na( DFO_hatchery$STOCK_GFE_ID)
DFO_hatchery[cond,c("STOCK_GFE_ID","STOCK_LATITUDE","STOCK_LONGITUDE")]
```
...
The last focal series `r paste("CO_46170","54283",sep = " - ")` is removed because it does not have coordinates, it is unclear what "(DOT)" stands for and there is only two data points.
...
In case i = `r i`: the focal series `r paste("CN_48448","443",sep = " - ")` is added to **CUSS** instead of being merged to the alternative series because the two series are not compatible and in two different locations.
...
In the cases (i = `r paste0(is,collapse = ", ")`): there are two recurrent `GFE_ID`s of focal series that seem to be be the same locations because they have the same `WATERBODY` in **NUSEDS**:
...
dupli <- duplicated(conservation_unit_system_sites)
conservation_unit_system_sites[dupli,]  # 	441	EAGLE CREEK CM_47906
...
```{r, include=FALSE}
nrow(nuseds_final)     # 311957 310644 310644 310538 309727 307217
nrow(all_areas_nuseds) # 311957 310644 310644 310538 309727 307217
nrow(conservation_unit_system_sites) # 7036 6955
```
