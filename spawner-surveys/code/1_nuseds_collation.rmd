...
Here there is one instance of a location (`GFE_ID`) that does not have values for `FAZ_ACRO`,`MAZ_ACRO`,`JAZ_ACRO`:

```{r,echo=F}

# Are there GFE_IDs associated to mutlile zones: --> just one and it is a mistake
for(val in GFE_IDs){
...
Note that we do not remove the references of these series (i.e., their `IndexId` & `GFE_ID` association) in **CUSS** at this stage because they could be alternative series for the series in **NUSEDS** that do not appear in **CUSS**. It is only once series in **NUSEDS** absent in **CUSS** were found alternative series for, that we remove series in **CUSS** that have only NAs and/or 0s in **NUSEDS**.

## Fix populations in NUSEDS that are not in CUSS

The goal is to look for each `IndexId/POP_ID` & `GFE_ID` series in **NUSEDS** that are not in **CUSS** and to operate as follow:
...
2) The `FULL_CU_IN` (and consequently `CU_NAME` and other related fields) of the remaining series is found by intersecting the coordinates (`X_LONGT` and `Y_LAT`) associated with the `GFE_ID` with the PSE's CUs' shape files. This step might require to define certain coordinates manually if not available. Once associated to a CU, these series are added to **CUSS**.
...
There are `r nrow(trackRecord_nuseds_nocuss)` `IndexId` & `GFE_ID` references in **NUSEDS** that are not present in **CUSS**, among which `r sum(trackRecord_nuseds_nocuss$FULL_CU_IN != "")` already have a `FULL_CU_IN`.
...
Here are the `r t["Manually defined"]` locations with coordinates we manually defined using Google Maps or any resource we could find.
...
show <- unique(all_areas_nuseds[cond,c("WATERBODY","GFE_ID","POPULATION")])
rownames(show) <- NULL
show
```
...
By comparing all the time series having the same `indexId/POP_ID` we can see all of them are comparable: 
...
```
 ...
 We correct in to `CARIBOO RIVER - LOWER` in **NUSEDS** to match we other notations of the same river:
...
i <- i + 1 
cond <- trackRecord_nuseds_nocuss$GFE_ID == GFE_IDs_here[i]
trackRecord_nuseds_nocuss$WATERBODY[cond] |> unique() # SPROAT RIVER BELOW FALLS TODO
...
We identify the CU they belong to using the CUs' shape files of the PSE. Series with `X_LONGT` and `Y_LAT` values that intersects one unique CU layer of the same species are attributed the CU's `FULL_CU_IN`.
...
# trackRecord_nuseds_nocuss_copy <- trackRecord_nuseds_nocuss
# trackRecord_nuseds_nocuss <- trackRecord_nuseds_nocuss_copy
  ...
  if(FULL_CU_IN != ""){ # find the cuid and cu_name_pse from decoder
    ...
    if(length(layer_rg[[1]]) > 1){
      ...
      if(!any(cond_rg_sp)){
        ...
        # This Chum popuation falls outside the only Fraser Chum CU
        ...
        cond_CO_2236 <- trackRecord_nuseds_nocuss$IndexId[r] == "CO_3027" # & GFE_ID == 2655
        ...
        cond_SEL_2792 <- trackRecord_nuseds_nocuss$IndexId[r] == "SX_7805" # & GFE_ID == 2792
        ...
        cond_SEL_64745 <- trackRecord_nuseds_nocuss$IndexId[r] == "SX_7818" # & GFE_ID == 64745
        #' here POPULATION = "Atnarko Lakes Sockeye"
        #' CU choices = "Northern Coastal Fjords" "South Atnarko Lakes"
        #' --> "South Atnarko Lakes" selected
        
        cond_SEL_1303 <- trackRecord_nuseds_nocuss$IndexId[r] == "SX_52080" # & GFE_ID == 1303
        #' here POPULATION = "Kennedy Lake Sockeye"
        #' CU choices =  "West Vancouver Island" "Kennedy"
        #' --> "Kennedy" selected
        
        cond_SER_1307 <- trackRecord_nuseds_nocuss$IndexId[r] == "SX_52120" # & GFE_ID == 1307
        #' here POPULATION = "Clayoquot Arm Beaches Sockeye"
        #' CU choices = "West Vancouver Island" "Kennedy" 
        #' --> "West Vancouver Island" selected
        
        cond_CM_2252 <- trackRecord_nuseds_nocuss$IndexId[r] == "CM_7775" # & GFE_ID == 2252
        ...
        cond_SX_45013 <- trackRecord_nuseds_nocuss$IndexId[r] == "SX_45013"
        ...
        cond_SX_46248 <- trackRecord_nuseds_nocuss$IndexId[r] == "SX_46248"
        ...
        if(length(layer_CU[[1]]) > 1){
          ...
          }else if(cond_SX_45013 | cond_SX_46248 | cond_SX_46268 | cond_SX_46308){
...
```{r, include=FALSE}
#'* Now I have to find the CU_NAME and FULL_CU_IN from CUSS *
#'
...
for(cuid in cuids){
  ...
  if(!any(cond_cuss)){
    print(paste0("(r = ",r,") CU not found for: ",species_name," - ",cu_name_pse," - ",cu_name_dfo," - ",cuid))
...
There are `r length(cuids)` CUs (`r sum(cond)` time series) having a `cu_name_pse` and `cuid` but that are not in **CUSS**:
...
for(cuid in cuids){
  ...
  print(paste0("The series below is associated to CU ",cu_name_pse," (cuid = ",cuid,") in the PSE:"))
...
Finally, there are `r nrow(show)` time series with a `CU_NAME` and `FULL_CU_IN` that are not in **CUSS** and for which we could not find the corresponding `cuid` and `cu_name_pse`:
...
All these time series will be added to **CUSS** in the next step.

### Add the references of the missing time series to CUSS

```{r, echo=F}
# Series with FULL_CU_IN 
...
First we add to **CUSS** the references of the `r sum(cond_withCU)` time series for which we found the `CU_NAME` and `FULL_CU_IN`.

```{r, echo = TRUE}
# conservation_unit_census_sites_copy <- conservation_unit_census_sites
# conservation_unit_census_sites <- conservation_unit_census_sites_copy
...
for(r in 1:nrow(data)){
  ...
  if(!any(cond_here)){
    ...
    cond_nuseds <- all_areas_nuseds_all$GFE_ID == GFE_ID
    cuss_new$WATERSHED_CDE <- all_areas_nuseds_all$WATERSHED_CDE[cond_nuseds] |> unique()
    cuss_new$FWA_WATERSHED_CDE <- all_areas_nuseds_all$FWA_WATERSHED_CDE[cond_nuseds] |> unique()
  }
...
Second we add to **CUSS** the references of the `r sum(cond_withoutCU)` time series for which we did not find the `CU_NAME` and `FULL_CU_IN` but we found the `cuid` and `cu_name_pse`.
...
Finally, there `r sum(cond_withoutCUID)` time series with a `CU_NAME` and `FULL_CU_IN` for which we could not find the corresponding `cuid` and `cu_name_pse` are added to **CUSS**.
...
So now are the time series in **NUSEDS** should be in **CUSS**, is that true?
...
Single `IndexId/POP_ID` associated to a multiple `GFE_ID`:
...
THIS IS WERE THE SCRIPT HAS BEEN SPLIT

###############################################################################



## Check problematic time series references in CUSS

Look for each time series references in **CUSS** (i.e., unique `IndexId` & `GFE_ID` association) and:
...
3)  else: that could be due to the wrong attribution of either (i) the `IndexId` or (ii) the `GFE_ID`. The rest of the code looks for potential alternative series in **NUSEDS** with either a different `IndexId` (but with the same species) or a different `GFE_ID`. Alternative series are by definition **NOT present in CUSS**.
...
if(import_file_locally_TEMPORARY){
  ...
  if(remove_timeSeries_zeros_too){
    ...
    trackRecord <- read.csv(paste0(wd_data,"/trackRecord_noNA0s_TEMP.csv"), header = T)
    
  }else{
    
    trackRecord <- read.csv(paste0(wd_data,"/trackRecord_noNAs_TEMP.csv"), header = T)
  ...
  if(remove_timeSeries_zeros_too){
      trackRecord <- cuss_nuseds_match_parallel_fun(conservation_unit_census_sites = conservation_unit_census_sites,  # previously conservation_unit_system_sites
                                                all_areas_nuseds = all_areas_nuseds,
                                                cores_nb = cores_nb)
...
nrow(trackRecord_cuss_alternative) # 3 3 4
...
#' TODO: we remove PKE_42409 & GFE_ID = 1490 from conservation_unit_census_sites
#' and PKE_54793433 & GFE_ID = 1490 from NUSEDS.
# removed <- data.frame(IndexId = c("PKE_42409","PKE_54793433"),
#                       GFE_ID = c(1490,1490))
# removed$dataset <- c("CUSS","NUSEDS")
# removed$comment <- c("Removed because not in NUSEDS and alternative series PKE_54793433 & GFE_ID = 1490 has only one data point, which was also removed",
#                      "Only 1 data point(s) and not in CUSS")
# 
removed_all <- rbind(removed_all,removed)
# 
# condition <- conservation_unit_census_sites$IndexId == removed$IndexId[1] &
#   conservation_unit_census_sites$GFE_ID == removed$GFE_ID[1]
# conservation_unit_census_sites[condition,]
# conservation_unit_census_sites <- conservation_unit_census_sites[!condition,]
# nrow(conservation_unit_census_sites) # 7144
# 
...
```{r, echo = F}
cond <- conservation_unit_census_sites$IndexId %in% c("CN_46842","CN_46841")
unique(conservation_unit_census_sites$CU_NAME[cond])
```
...
```{r,include=F}
cond <- conservation_unit_census_sites$GFE_ID == as.numeric(gfeids[2])
Y_LAT_focal <- conservation_unit_census_sites$Y_LAT[cond] |> unique()
X_LONGT_focal <- conservation_unit_census_sites$X_LONGT[cond] |> unique()
SYSTEM_SITE_focal <- conservation_unit_census_sites$SYSTEM_SITE[cond] |> unique()

cond <- conservation_unit_census_sites$GFE_ID == as.numeric(gfeids[1])
Y_LAT <- conservation_unit_census_sites$Y_LAT[cond] |> unique()
X_LONGT <- conservation_unit_census_sites$X_LONGT[cond] |> unique()
SYSTEM_SITE <- conservation_unit_census_sites$SYSTEM_SITE[cond] |> unique()
...
```{r,echo=FALSE}
cond <- conservation_unit_census_sites$GFE_ID %in% as.numeric(gfeids)
show <- unique(conservation_unit_census_sites[cond,c("CENSUS_SITE","GFE_ID","X_LONGT","Y_LAT")]) # SYSTEM_SITE
rownames(show) <- NULL
show
```
...
```{r,echo=F}
# cond <- conservation_unit_census_sites$GFE_ID %in% c(2463,285)
# unique(conservation_unit_census_sites$SYSTEM_SITE[cond])
```

```{r,include=F}
# cond <- conservation_unit_census_sites$GFE_ID == 285
# Y_LAT_focal <- conservation_unit_census_sites$Y_LAT[cond] |> unique()
# X_LONGT_focal <- conservation_unit_census_sites$X_LONGT[cond] |> unique()
# 
# cond <- conservation_unit_census_sites$GFE_ID == 2463
# Y_LAT <- conservation_unit_census_sites$Y_LAT[cond] |> unique()
# X_LONGT <- conservation_unit_census_sites$X_LONGT[cond] |> unique()
# 
# # c(Y_LAT_focal,X_LONGT_focal)
...
#                                                 GFE_ID_alter = 285,
#                                                 all_areas_nuseds = all_areas_nuseds,
#                                                 conservation_unit_census_sites = conservation_unit_census_sites)
```
...
sapply(X = c("SX_7763","SX_47954"),function(iid){
  cond <- conservation_unit_census_sites$IndexId == iid
  return(conservation_unit_census_sites$CU_NAME[cond])
})
```
...
nrow(trackRecord_cuss_noAlernative) # 243 243 254
...
for(r in 1:nrow(trackRecord_cuss_noAlernative)){
  ...
  # remove series from conservation_unit_census_sites
  cond <- conservation_unit_census_sites$IndexId == iid &
    conservation_unit_census_sites$GFE_ID == gfeid
  conservation_unit_census_sites <- conservation_unit_census_sites[!cond,]
}

nrow(conservation_unit_census_sites) # 6906 6890


# all but a few of the series are in all_areas_nuseds_all
cond <- removed_all$dataset == "both"
sum(cond) # 241 (vs. 243) ; 253 (vs. 254)

```

`r sum(cond)` of the `r nrow(trackRecord_cuss_noAlernative)` `IndexId` & `GFE_ID` references were removed previously from **NUSEDS** because they contained only NAs and/or 0s. Only the following references were not in **NUSEDS** initially:
...
```{r, echo=F}
# find the references
cond <- rep(F,nrow(conservation_unit_census_sites_all))
for(i in 1:length(IndexId)){
  cond_i <- conservation_unit_census_sites_all$IndexId == IndexId[i] & 
  conservation_unit_census_sites_all$GFE_ID == GFE_ID[i]
  cond <- cond | cond_i
}

show <- conservation_unit_census_sites_all[cond,c("SPECIES_QUALIFIED","CU_NAME","FULL_CU_IN","GFE_ID","CENSUS_SITE")] # "UPPER FRASER"
rownames(show) <- NULL
show
```
...
cond <- trackRecord$in_nused == "no"
trackRecord_cuss_multi_fgeid <- trackRecord[grepl(comment,trackRecord$comment) & cond,]
```

This happens for `r length(unique(trackRecord_cuss_multi_fgeid$IndexId))` population:
...
There are two time series in **NUSEDS** with same `IndexID/POP_ID` but different `GFE_ID` and they are both in **CUSS** as well:

```{r, echo=FALSE}
cond <- trackRecord$IndexId == "CO_46835" # & trackRecord$GFE_ID == 2462
show <- trackRecord[cond,c("IndexId","GFE_ID","in_cuss","in_nused")]
rownames(show) <- NULL

show$CENSUS_SITE <- sapply(show$GFE_ID,function(gfeid){
  cond <- conservation_unit_census_sites$GFE_ID == gfeid
  return(unique(conservation_unit_census_sites$CENSUS_SITE[cond]))
})

# if the GFE_ID 285 connected to other POP_IDs/IndexIds
cond <- conservation_unit_census_sites_all$GFE_ID == 2462
unique(conservation_unit_census_sites_all$IndexId[cond])

show
```

We can consequently remove this problematic reference from **CUSS**.

```{r, include=TRUE}
removed <- data.frame(IndexId = c("CO_46835"),
                      GFE_ID = 2462)
removed$dataset <- "CUSS"
removed$comment <- "Not in NUSEDS and no alternative reference"
removed_all <- rbind(removed_all,removed)

# remove from CUSS
cond <- conservation_unit_census_sites$IndexId == "CO_46835" & 
  conservation_unit_census_sites$GFE_ID == 2462

conservation_unit_census_sites <- conservation_unit_census_sites[! cond,]
nrow(conservation_unit_census_sites) # 6905

```

```{r, include= FALSE}

#'* CHECK: all references in CUSS should be in NUSEDS *

IndexId_GFE_ID_CUSS <- paste(conservation_unit_census_sites$IndexId,
                             conservation_unit_census_sites$GFE_ID,sep = "_")

IndexId_GFE_ID_NUSEDS <- paste(all_areas_nuseds$IndexId,
                               all_areas_nuseds$GFE_ID,sep = "_") |> unique()


cond <- ! IndexId_GFE_ID_CUSS %in% IndexId_GFE_ID_NUSEDS
...
series_nuseds <- paste(all_areas_nuseds$IndexId,
                       all_areas_nuseds$GFE_ID,sep = "&")
series_nuseds <- unique(series_nuseds)
length(series_nuseds) # 7082 7060

series_cuss <- paste(conservation_unit_census_sites$IndexId,
                     conservation_unit_census_sites$GFE_ID,sep = "&")
series_cuss <- unique(series_cuss)
length(series_cuss) # 6894 6889
...
series_Nuseds_noCuss <- series_nuseds[! series_nuseds %in% series_cuss]
series_Nuseds_noCuss <- series_Nuseds_noCuss[order(series_Nuseds_noCuss)]
n <- length(series_Nuseds_noCuss) # 188 171
n
```
...
for(i in 1:nrow(trackRecord_nuseds)){
  ...
  if(nrow(nuseds_here) == 0){
    ...
    for(gfeid_i in gfeids){
      # gfeid_i <- gfeids[2]
      cond <- conservation_unit_census_sites$IndexId == iid &
        conservation_unit_census_sites$GFE_ID == gfeid_i
      
      # if the alternative series is in CUSS
      if(any(cond)){
        alternative_GFE_ID <- c(alternative_GFE_ID,
                                conservation_unit_census_sites$GFE_ID[cond])
  ...
  if(nrow(nuseds_here) == 0){
    ...
    for(iid_i in iids){
      # iid_i <- iids[1]
      cond <- conservation_unit_census_sites$IndexId == iid_i &
        conservation_unit_census_sites$GFE_ID == gfeid
      
      # if the alternative series is in CUSS
      if(any(cond)){
        alternative_IndexId <- c(alternative_IndexId,
                                 conservation_unit_census_sites$IndexId[cond])
...
nrow(trackRecord_nuseds) # 188 171
rownames(trackRecord_nuseds) <- NULL
```
...
nrow(all_areas_nuseds) # 312302 308231 308232
```
...
cond <- all_areas_nuseds$IndexId == iid & 
  all_areas_nuseds$GFE_ID == gfeid
all_areas_nuseds <- all_areas_nuseds[!cond,]
nrow(all_areas_nuseds) # 312266 308195
```
...
#' (Summer 4-1) in CUSS:
cond <- conservation_unit_census_sites_all$CU_NAME == toupper(cu_decoder_here$cu_name_dfo) # does not work
sum(cond) # 0

cond <- grepl(toupper("East Vancouver Island-Georgia Strait"),
              conservation_unit_census_sites_all$CU_NAME) & 
  conservation_unit_census_sites_all$SPECIES_QUALIFIED == "CK"
conservation_unit_census_sites_all[cond,]
```

```{r, echo=F}
unique(conservation_unit_census_sites_all$CU_NAME[cond])
```
...
cu_fields_info <- unique(conservation_unit_census_sites_all[cond,cu_fields])
...
```{r, include = F}
conservation_unit_census_sites <- rbind(conservation_unit_census_sites,
                                        cuss_new)
...
cond <- conservation_unit_census_sites$IndexId %in% iida
...
cond <- conservation_unit_census_sites$GFE_ID == gfeid
SYSTEM_SITE <- conservation_unit_census_sites$SYSTEM_SITE[cond]
cond <- conservation_unit_census_sites$IndexId %in% c(iid,iid_alter)
CU_NAME <- unique(conservation_unit_census_sites$CU_NAME[cond])
...
conservation_unit_census_sites <- fields_edit_NUSEDS_CUSS_fun(edit_CUSS = T, 
...
cond <- conservation_unit_census_sites$IndexId == iid_alter & conservation_unit_census_sites$GFE_ID == gfeid
conservation_unit_census_sites[cond,]
# 50.93509	-118.801294
...
cond <- grepl("SHUSWAP",conservation_unit_census_sites$CU_NAME) & conservation_unit_census_sites$SPECIES == "Sockeye"
conservation_unit_census_sites[cond,]$CU_NAME |> unique()
...
cond <- conservation_unit_census_sites $CU_NAME == "SHUSWAP COMPLEX-LATE TIMING"
for(f in c("CU_NAME","CU_ACRO","CU_LAT","CU_LONGT","CU_TYPE","CU_INDEX","FULL_CU_IN")){
  cuss_new[,f] <- unique(conservation_unit_census_sites[cond,f])
}
...
cond <- grepl("MOMICH",conservation_unit_census_sites$CU_NAME) & conservation_unit_census_sites$SPECIES == "Sockeye"
conservation_unit_census_sites[cond,]$CU_NAME |> unique()
# "SHUSWAP COMPLEX-EARLY SUMMER TIMING" "SHUSWAP COMPLEX-LATE TIMING"
...
cond <- conservation_unit_census_sites $CU_NAME == "SHUSWAP COMPLEX-EARLY SUMMER TIMING"
for(f in c("CU_NAME","CU_ACRO","CU_LAT","CU_LONGT","CU_TYPE","CU_INDEX","FULL_CU_IN")){
  cuss_new[,f] <- unique(conservation_unit_census_sites[cond,f])
}
...
# Decision: https://salmonwatersheds.slack.com/archives/CJ5RVHVCG/p1740166630290809
cond <- conservation_unit_census_sites$IndexId == iid_alter
CU_NAME_alter <- conservation_unit_census_sites$CU_NAME[cond] |> unique()
CU_NAME_alter
# "FRASER-EARLY SUMMER TIMING"

cond <- conservation_unit_census_sites$IndexId == iid_alter
FULL_CU_IN_alter <- conservation_unit_census_sites$FULL_CU_IN[cond] |> unique()
FULL_CU_IN_alter
# "SEL-06-06"
...
cond <- conservation_unit_census_sites$FULL_CU_IN == FULL_CU_IN_other
CU_NAME_other <- conservation_unit_census_sites$CU_NAME[cond] |> unique()
...
nrow(trackRecord_nuseds_gfeid) # 64 62 35 34
...
# We consequently attribute coordinates to this location using Google Maps. We select the location just above Stamp Falls, on the left of the Stamp river fish ladder:
...
```{r, include=FALSE}
# add to conservation_unit_census_sites
...
In case i = `r i`: series `r paste("SX_3302","3416",sep = " - ")` in black (bottom plot) is merged to alternative series `r paste("SX_3310","3416",sep = " - ")` in red and series `r paste("SX_3302","3444",sep = " - ")` in blue (bottom plot) is merged to alternative series `r paste("SX_3325","3444",sep = " - ")` in green.
...
cond <- conservation_unit_census_sites$GFE_ID == 129
unique(conservation_unit_census_sites[cond,c("SYSTEM_SITE","Y_LAT","X_LONGT")])
# PORTAGE CREEK 50.7083513	-122.269389	

cond <- conservation_unit_census_sites$GFE_ID == 2451
unique(conservation_unit_census_sites[cond,c("SYSTEM_SITE","Y_LAT","X_LONGT")])
...
# it is not in the original conservation_unit_census_sites_all:
cond <- conservation_unit_census_sites_all$GFE_ID == 31516
sum(cond) 
...
# it is not in the original conservation_unit_census_sites_all:
cond <- conservation_unit_census_sites_all$GFE_ID == 31740
sum(cond) 
...
#' conservation_unit_census_sites
...
cond <- conservation_unit_census_sites$IndexId %in% c("CN_47277","CN_2483")
CU_NAMES <- (conservation_unit_census_sites$CU_NAME[cond])
legend("topright",CU_NAMES,bty = "n")
mtext(text = "CU_NAME",side = 3,adj = 1)
```

```{r,echo=FALSE}
cond <- conservation_unit_census_sites$GFE_ID %in% c(2464,142)
show <- conservation_unit_census_sites[cond,c("SYSTEM_SITE","GFE_ID","X_LONGT","Y_LAT")] |> unique()
rownames(show) <- NULL
```
...
show <- DFO_All_Streams_Segments[cond,]
rownames(show) <- NULL
show
```
...
cond <- conservation_unit_census_sites$IndexId == "CN_47277" &
  conservation_unit_census_sites$GFE_ID == 142
...
```{r, echo=FALSE}
# The coordinates are not in DFO_All_Streams_Segments:
cond <- conservation_unit_census_sites$ID == 54282
conservation_unit_census_sites[cond,c("SYSTEM_SITE","GFE_ID","Y_LAT","X_LONGT")]
```
...
cond <- conservation_unit_census_sites$GFE_ID %in% c(2119,2193)
conservation_unit_census_sites[cond,c("SYSTEM_SITE","GFE_ID","Y_LAT","X_LONGT")] |> unique()
...
cond <- conservation_unit_census_sites$IndexId %in% c("CO_46240","CO_46250")
conservation_unit_census_sites[cond,c("CU_NAME","IndexId")]
CU_NAME <- unique(conservation_unit_census_sites[cond,c("CU_NAME")])
```
...
```{r, include=FALSE}
cond <- conservation_unit_census_sites$GFE_ID %in% c(220,3477)
conservation_unit_census_sites[cond,c("SYSTEM_SITE","GFE_ID","Y_LAT","X_LONGT")] |> unique()
```
...
cond <- conservation_unit_census_sites$GFE_ID %in% c(gfeid,gfeid_a)
conservation_unit_census_sites[cond,c("SYSTEM_SITE","GFE_ID","Y_LAT","X_LONGT")] |> unique()
...
sum(conservation_unit_census_sites_all$GFE_ID == 2451)
...
cond <- conservation_unit_census_sites$GFE_ID %in% c(gfeid)
SYSTEM_SITE <- conservation_unit_census_sites$SYSTEM_SITE[cond] |> unique()
```
...
cond <- conservation_unit_census_sites$GFE_ID %in% c(261,2746)
conservation_unit_census_sites[cond,c("SYSTEM_SITE","GFE_ID","Y_LAT","X_LONGT")]
```
...
cond <- conservation_unit_census_sites$GFE_ID %in% c(gfeid,gfeid_a)
show <- unique(conservation_unit_census_sites[cond,c("SYSTEM_SITE","GFE_ID","Y_LAT","X_LONGT")])
rownames(show) <- NULL
```
...
cond <- conservation_unit_census_sites$GFE_ID %in% c(446068199,2618)
conservation_unit_census_sites[cond,c("SYSTEM_SITE","GFE_ID","Y_LAT","X_LONGT")] |> unique()
...
nb_rows <- nrow(trackRecord_nuseds_nocuss) # 117 41 37
nb_rows
rownames(trackRecord_nuseds_nocuss) <- NULL
```
...
for(r in 1:nrow(trackRecord_nuseds_nocuss)){
  ...
  if((is.na(X_LONGT) | is.na(Y_LAT)) & GFE_ID %in% conservation_unit_census_sites$GFE_ID){
    cond <- conservation_unit_census_sites$GFE_ID == GFE_ID
    X_LONGT <- conservation_unit_census_sites$X_LONGT[cond] |> as.numeric() |> unique()
    Y_LAT <- conservation_unit_census_sites$Y_LAT[cond] |> as.numeric() |> unique()
    SYSTEM_SITE <- conservation_unit_census_sites$SYSTEM_SITE[cond] |> unique()
  }
  
  if((is.na(X_LONGT) | is.na(Y_LAT)) & GFE_ID %in% conservation_unit_census_sites_all$GFE_ID){
    cond <- conservation_unit_census_sites_all$GFE_ID == GFE_ID
    X_LONGT <- conservation_unit_census_sites_all$X_LONGT[cond] |> as.numeric() |> unique()
    Y_LAT <- conservation_unit_census_sites_all$Y_LAT[cond] |> as.numeric() |> unique()
    SYSTEM_SITE <- conservation_unit_census_sites_all$SYSTEM_SITE[cond] |> unique()
  }
...
cond <- grepl("STAMP",conservation_unit_census_sites$SYSTEM_SITE)
conservation_unit_census_sites[cond,c("SYSTEM_SITE","GFE_ID","Y_LAT","X_LONGT")] |> unique()
# point takien from Google Maps below the Falls
...
sum(trackRecord_nuseds_nocuss$GFE_ID %in% conservation_unit_census_sites_all$GFE_ID) # 24
sum(trackRecord_nuseds_nocuss$GFE_ID %in% DFO_All_Streams_Segments$ID) # 34

sum(trackRecord_nuseds_nocuss$GFE_ID %in% conservation_unit_census_sites$GFE_ID | trackRecord_nuseds_nocuss$GFE_ID %in% DFO_All_Streams_Segments$ID) # 35
...
CUSS_species_GFE_ID <- paste(conservation_unit_census_sites$SPECIES,
                             conservation_unit_census_sites$GFE_ID,sep = "_")
...
for(r in 1:nrow(trackRecord_nuseds_nocuss)){
  ...
  cond <- conservation_unit_census_sites$IndexId == trackRecord_nuseds_nocuss$IndexId[r]
  if(sum(cond) > 0){
    IndexId_inCuss <- T
  }else{
    IndexId_inCuss <- F
  ...
  if(is.na(FULL_CU_IN) & is.na(cuid)){
    ...
    all_areas_nuseds <- remove_rows_fields_fun(dataframe = all_areas_nuseds,
      ...
      if(is.na(FULL_CU_IN) | is.na(cuss_new$FULL_CU_IN)){
        ...
        cond <- conservation_unit_census_sites$FULL_CU_IN == FULL_CU_IN
        cu_fields_info <- unique(conservation_unit_census_sites[cond,cu_fields])
...
cond <- is.na(conservation_unit_census_sites$Y_LAT) | is.na(conservation_unit_census_sites$X_LONGT)
sum(cond) # 0
```
...
```{r, include=F}
#' There are multiple locations in conservation_unit_census_sites that have 
...
conservation_unit_census_sites$X_LONGT <- round(as.numeric(conservation_unit_census_sites$X_LONGT),6)
conservation_unit_census_sites$Y_LAT <- round(as.numeric(conservation_unit_census_sites$Y_LAT),6)

# Find the coordinates of all the GFE_IDs
locations <- unique(conservation_unit_census_sites[,c("GFE_ID","X_LONGT","Y_LAT")])
...
for(r in 1:nrow(coord_duplicated)){
  cond <- conservation_unit_census_sites$X_LONGT == coord_duplicated[r,"X_LONGT"] &
    conservation_unit_census_sites$Y_LAT == coord_duplicated[r,"Y_LAT"]
  out <- unique(conservation_unit_census_sites[cond,c("SYSTEM_SITE","GFE_ID","Y_LAT","X_LONGT")])
...
```{r, include=F}
cond <- conservation_unit_census_sites$SYSTEM_SITE == "BAXTER BEACH"
cond <- conservation_unit_census_sites$SYSTEM_SITE == "BEAR BEACH - SHORE"
cond <- conservation_unit_census_sites$SYSTEM_SITE == "BETTY FRANK'S - SHORE"
cond <- conservation_unit_census_sites$SYSTEM_SITE %in% c("BAXTER BEACH",
                                                          "BEAR BEACH - SHORE",
                                                          "BETTY FRANK'S - SHORE")
conservation_unit_census_sites[cond,]
```
...
```{r, include=F}
cond <- conservation_unit_census_sites$SYSTEM_SITE == "LILLOOET RIVER"
conservation_unit_census_sites[cond,]
```
...
```{r, include=F}
cond <- conservation_unit_census_sites$SYSTEM_SITE == "BETTY FRANK'S - SHORE"
cond <- conservation_unit_census_sites$SYSTEM_SITE %in% c("FORESTRY CREEK",
                                                          "MARTY'S CREEK")
conservation_unit_census_sites[cond,]
```
...
for(l in unique(locations_duplicated$group)){
  ...
  for(i in 1:length(GFE_ID)){
    cond_cuss <- conservation_unit_census_sites$GFE_ID == GFE_ID[i] & 
      conservation_unit_census_sites$SYSTEM_SITE == SYSTEM_SITE[i]
    
    conservation_unit_census_sites$Y_LAT[cond_cuss] <- Y_LAT_new[i]
    conservation_unit_census_sites$X_LONGT[cond_cuss] <- X_LONGT_new[i]
    conservation_unit_census_sites$coordinates_changed[cond_cuss] <- T
  }
}

locations <- unique(conservation_unit_census_sites [,c("GFE_ID","X_LONGT","Y_LAT")])
nrow(locations) # 2361 2345 2324 2333
```
...
```{r, include=FALSE}
# there is one duplicated row in conservation_unit_census_sites, not sure why

dupli <- duplicated(conservation_unit_census_sites)
conservation_unit_census_sites[dupli,]  # 
conservation_unit_census_sites <- conservation_unit_census_sites[!dupli,]
```
...
nrow(conservation_unit_census_sites) # 7036 6955
```
...
nb_cases <- sum(cond) # 3453 3263 2996
...
# write.csv(conservation_unit_census_sites,paste0(wd_data,"/conservation_unit_census_sites_TEMP.csv"), row.names = F)
...
# conservation_unit_census_sites <- read.csv(paste0(wd_data,"/conservation_unit_census_sites_TEMP.csv"), header = T)
...
if(export_datasets){
  ...
  log_file_new$conservation_unit_census_sites_file_name <- conservation_unit_census_sites_file_name
