...
# To avoid having to wait too much time where executing the code multiple times.
...
nrow(all_areas_nuseds_all) # 415230 412492
...
if(remove_timeSeries_zeros_too){
  ...
  print("Time series uniqually made of both NAs are removed.")
}
```
...
nrow(all_areas_nuseds) # 312331 309647
...
nrow(IndexId_GFE_ID_all) # 11568 11553

IndexId_GFE_ID <- unique(all_areas_nuseds[,c("IndexId","GFE_ID")])
nrow(IndexId_GFE_ID) # 7084 7062
...
length(IndexId_GFE_ID_removed_c) # 4484 4491
...
### Remove series number of data points < 4 BRUNO: TO REMOVE

```{r, include=FALSE}
cond_3 <- trackRecord_nuseds$nb_dataPt < 4
trackRecord_nuseds_3 <- trackRecord_nuseds[cond_3,]
nrow(trackRecord_nuseds_3) # 105 94
...
trackRecord_nuseds_3$year_max <- NA
for(r in 1:nrow(trackRecord_nuseds_3)){
  # r <- 1
  cond <- all_areas_nuseds$IndexId == trackRecord_nuseds_3$IndexId[r] &
    all_areas_nuseds$GFE_ID == trackRecord_nuseds_3$GFE_ID[r]
  
  cond_NA <- is.na(all_areas_nuseds$MAX_ESTIMATE)
  
  yr_max <- max(all_areas_nuseds$Year[cond & !cond_NA])
  
  trackRecord_nuseds_3$year_max[r] <- yr_max
}
...
# nrow(all_areas_nuseds) # 310846 308250
# 
# removed_all <- rbind(removed_all,removed)
# nrow(removed_all) # 4597 4593
# 
# nrow_after <- nrow(all_areas_nuseds)
```
...
nrow(trackRecord_nuseds_gfeid) # 62 35 34
...
```{r,echo=FALSE}
cond <- DFO_hatchery$STOCK_GFE_ID == as.numeric(gfeid) & !is.na(DFO_hatchery$STOCK_GFE_ID)
...
In case i = `r i`: it is a special situation deserving more attention. There are two different issues related to `CN_47277`: (1) it is associated to two locations (`GFE_IDs`) and (2) in one location (`GFE_ID = 142`), there is another time series with `IndexId = # Fix dupl`, which belong to the same CU:
...
**DECISION**: we decide to merge the alternative series `CN_47277 - 142` to the focal series `CN_47277 - 2464` in **NUSEDS** and to add `CN_47277 - 2464` to **CUSS**:
...
nb_rows <- nrow(trackRecord_nuseds_nocuss) # 41 37
nb_rows
...
for(r in 1:nrow(trackRecord_nuseds_nocuss)){
  ...
  if(GFE_ID %in% DFO_All_Streams_Segments$ID){
    ...
    SYSTEM_SITE <- DFO_All_Streams_Segments$NME[cond]
  }
  ...
  if((is.na(X_LONGT) | is.na(Y_LAT)) & GFE_ID %in% conservation_unit_system_sites_all$GFE_ID){
    ...
    X_LONGT <- conservation_unit_system_sites_all$X_LONGT[cond] |> as.numeric()
    Y_LAT <- conservation_unit_system_sites_all$Y_LAT[cond] |> as.numeric()
    SYSTEM_SITE <- conservation_unit_system_sites_all$SYSTEM_SITE[cond]
  }
...
# "Mclellan Creek (Bella Coola) Coho"
...
```{r, echo=FALSE}
unique(trackRecord_nuseds_nocuss[cond_NA,c("GFE_ID","WATERBODY","X_LONGT","Y_LAT","coordinates_changed")])
```
...
sum(trackRecord_nuseds_nocuss$GFE_ID %in% conservation_unit_system_sites_all$GFE_ID) # 24
sum(trackRecord_nuseds_nocuss$GFE_ID %in% DFO_All_Streams_Segments$ID) # 34

sum(trackRecord_nuseds_nocuss$GFE_ID %in% conservation_unit_system_sites$GFE_ID | trackRecord_nuseds_nocuss$GFE_ID %in% DFO_All_Streams_Segments$ID) # 35
...
for(r in 1:nrow(trackRecord_nuseds_nocuss)){
  ...
  X_LONGT <- trackRecord_nuseds_nocuss$Y_LAT[r]
  comment <- NA
    ...
    if(length(layer_rg[[1]]) > 1){
      ...
      if(!any(cond_rg_sp)){
        ...
        if(length(layer_CU[[1]]) == 0){ # no match, try to buffer
          layer_CU <- st_intersects(point, st_buffer(x =  CUs_gdb[cond_rg_sp,], dist = .05))
        }
        
        # plot(st_geometry(regions_shp[layer_rg,]))
        # plot(st_geometry(CUs_gdb_rg_sp[layer_CU[[1]],]), add = T, col = alpha("blue",.3))
        ...
        }else if(length(layer_CU[[1]]) > 1){
          print("More than one CU in CUs_GBD")
          # break
          
          CUs <- CUs_gdb[cond_rg_sp,]$CU_NAME[layer_CU[[1]]]
          
          IndexId <-  trackRecord_nuseds_nocuss$IndexId[r]
          WATERBODY <- trackRecord_nuseds_nocuss$WATERBODY[r]
          
          cond1 <- IndexId == "SX_52080" & WATERBODY == "KENNEDY LAKE BEACHES" & any(grepl("Kennedy",CUs))
          
          cond2 <- IndexId == "SX_52120" & WATERBODY == "CLAYOQUOT ARM BEACHES" & any(grepl("West Vancouver Island",CUs))
          
          if(cond1){
            # check
            # cond <- all_areas_nuseds$IndexId == "SX_52080"
            # unique(all_areas_nuseds$POPULATION[cond]) # "Kennedy Lake Sockeye"
            layer_CU[[1]] <- layer_CU[[1]][grepl("Kennedy",CUs)]
            
          }else if(cond2){
            # check
            # cond <- all_areas_nuseds$IndexId == "SX_52120"
            # unique(all_areas_nuseds$POPULATION[cond]) # "Clayoquot Arm Beaches Sockeye"
            layer_CU[[1]] <- layer_CU[[1]][grepl("West Vancouver Island",CUs)]
        ...
        if(length(layer_CU[[1]]) > 1){
          comment <- paste(comment,"More than one CU layer and no sure which one to choose: ",sep = "; ")
...
series_nuseds <- paste(all_areas_nuseds$IndexId,
                       all_areas_nuseds$GFE_ID,sep = "&")
series_nuseds <- unique(series_nuseds)
length(series_nuseds) # 6953 6952 6947 6917 6911

series_cuss <- paste(conservation_unit_system_sites$IndexId,
                     conservation_unit_system_sites$GFE_ID,sep = "&")
series_cuss <- unique(series_cuss)
length(series_cuss) # 6953 6952 6947 6917 6911
...
if(update_coordinates){
  ...
  cond <- locations_duplicated$group == letter
  i_toChange <- 2
  # Location of the dam in the Nicola lake
  X_Y <- c(50.163045,-120.665873)
  locations_duplicated$Y_LAT_new[cond][i_toChange] <- X_Y[1]
  locations_duplicated$X_LONGT_new[cond][i_toChange] <- X_Y[2]
  show <- locations_duplicated[cond,c("GFE_ID","SYSTEM_SITE","Y_LAT","X_LONGT","Y_LAT_new","X_LONGT_new")]
  rownames(show) <- NULL
...
```{r, include=FALSE}
nrow(nuseds_final)     # 310644 310644 310538 309727 307217
nrow(all_areas_nuseds) # 310644 310644 310538 309727 307217
nrow(conservation_unit_system_sites) # 6955
```
...
By summing data points in a few cases, we assum that the different runs (e.g., “Chinook Run 1” and “Chinook Run 2”) can be considered a single population. For example, the Bridge River has Summer and Late run sockeye surveys, but these are both in the MIDDLE FRASER river-type sockeye CU and are summed to yield the Bridge River survey for that CU in the PSE.
