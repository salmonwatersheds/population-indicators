...
export_datasets <- F  # set tp true to update the datasets
```
...
So far we have not been able to determine which 0s are actual 0s and those which should be NAs. We consequently replace all 0s by NAs (`r paste0("n = ",nb_cases)`, or `r paste0(round(prop_cases,2),"%")` of the data points).
...
There are three instance with a special fix was done, as indicated below to corresponding figures.
