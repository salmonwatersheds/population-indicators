...
#' add the field var_in_MAX_ESTIMATE
...
# there are 971 Sockeye populations that are not in conservation_unit_system_sites
...
SFU_stream$dist[cond] <- apply(X = SFU_stream[cond,],
                                  MARGIN = 1,
                                  FUN = function(r){
                                    dist <- distance_Euclidean_fun(x_ref = r["Longitude"],
                                                                   y_ref = r["Latitude"], 
                                                                   x = r["X_LONGT"],
                                                                   y = r["Y_LAT"])
                                     return(round(dist,4))
...
# For dist, 0.1 ~ 10km
SFU_stream[cond,c("SFU_Stream_Name","Stream_Name_in_NuSEDS","Latitude","Longitude","GFE_ID","Y_LAT","X_LONGT","dist")]
...
for(r in 56:nrow(SFU_pop)){
  ...
  # if the location is in PSE:
  if(SFU_stream$in_PSE[cond_SFU_stream] & SFU_stream$in_PSE[cond_SFU_stream]){
  ...
  if(!Done){
    ...
    Longitude <- SFU_stream$Longitude[cond_SFU_stream]
    Latitude <- SFU_stream$Latitude[cond_SFU_stream]
