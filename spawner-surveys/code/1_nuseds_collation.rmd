...
In the case `i =` `r i` above, `r paste(iid,gfeid,sep = " - ")` time series is merged to the alternative series (`indexId` is changed from `r iid` to `r iid_alter` in **NUSEDS**) because the focal `POPULATION` (= `r POPULATION_focal`) is compatible with the alternative `CU_NAME` (= `r CU_NAME_alter`).
...
series_nuseds <- paste(all_areas_nuseds$IndexId,
                       all_areas_nuseds$GFE_ID,sep = "&")
series_nuseds <- unique(series_nuseds)
length(series_nuseds) # 6917 6911

series_cuss <- paste(conservation_unit_system_sites$IndexId,
                     conservation_unit_system_sites$GFE_ID,sep = "&")
series_cuss <- unique(series_cuss)
length(series_cuss) # 6917 6911
...
```{r, include=FALSE}
nrow(nuseds_final)     # 309727 307217
nrow(all_areas_nuseds) # 309727 307217
```
