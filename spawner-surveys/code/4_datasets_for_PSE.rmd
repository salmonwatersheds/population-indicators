...
TBR_data$GFE_ID <- NA
TBR_data$source_id <- NA

colnames(TBR_data)[colnames(TBR_data) == "survey_quality"] <- "stream_survey_quality"
colnames(TBR_data)[colnames(TBR_data) == "survey_method"] <- "stream_survey_method"
...
show <- unique(TBR_data[cond,c("region","species_name","cuid","cu_name_pse","stream_name_pse")])
show <- show[order(show$species_name),]
rownames(show) <- NULL
show
```
...
show <- unique(SH_data[,c("region","species_name","cuid","cu_name_pse")])
...
cond <- TBR_data$species_name == "Steelhead"
TBR_SH <- TBR_data[cond,]
# nrow(TBR_SH) # 29

cond <- SH_data$region == "Northern Transboundary"
SH_TBR <- SH_data[cond,]
# nrow(SH_TBR) # 29
...
if(!identical(TBR_SH$stream_observed_count,SH_TBR$stream_observed_count)){
  ...
  print("The SH data for TBR is present in both the SH and the TBR datasets; the data is identical; the data is removed from te TBR dataset.")
  
  cond <- TBR_data$species_name == "Steelhead"
  TBR_data <- TBR_data[!cond,]
}
...
```{r}
dataset2_spawner_surveys$source_id <- "NuSEDS"
```
