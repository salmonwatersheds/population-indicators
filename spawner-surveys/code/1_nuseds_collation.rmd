...
for(r in 1:nrow(trackRecord_nuseds_nocuss)){
  ...
  if(is.na(FULL_CU_IN)){
    ...
    all_areas_nuseds <- remove_rows_fields_fun(dataframe = all_areas_nuseds,
      ...
      toAdd <- data.frame(IndexId = trackRecord_nuseds_nocuss$IndexId[cond_i],
                          GFE_ID = trackRecord_nuseds_nocuss$GFE_ID[cond_i],
                          dataset = "CUSS",
                          CU_NAME = cuss_new$CU_NAME,
...
if(export_datasets){
  ...
  write.csv(log_file,paste0(wd_output,"/1_log_file.csv"),row.names = F)
