...
#'* Merge biostatus *
columns <- c("region","species_name","cuid",
...
cuid_old[!cuid_old %in% cuid_new] # 188
cond <- biostatus_old$cuid %in% cuid_old[!cuid_old %in% cuid_new]
biostatus_old[cond,] 
# Swan/Club: all good, was supposed to be removed
...
cond <- cuspawnerabundance$cuid == 510
cond <- cuspawnerabundance$cuid == 525
cond <- cuspawnerabundance$cuid == 985
...
#'* Different methods and why (8 out of 120) *
...
sum(cond_diff_method) # 16

# nb of CUs with with different method and output
sum(cond_diff_method & cond_diff) # 8
biostatus_merge[cond_diff_method & cond_diff,]

# 1) sr to percentile (14 overall, 7 with different biostatus)
cond_sr_to_percent <- cond_sr_old & cond_percent_new
sum(cond_sr_to_percent) # 14
...
# 2) percentile to sr (2 overall, 1 with different biostatus)
cond_percent_to_sr <- cond_sr_new & cond_percent_old
sum(cond_percent_to_sr) # 2
...
sum(cond_cuid_catchLow) # 1
cuid_catchLow <- dataset390_output[cond_390_cuid,]$cuid[cond_cuid_catchLow]

cond_cuid <- biostatus_merge_diff$cuid %in% cuid_catchLow
biostatus_merge_diff$explanation[cond_cuid] <- "rule 2: catch method quality != 1 ?"

#'* Biostatus improved or worsened (same method) (29 + 12 out of 120) *

cond_same_method <- (cond_sr_old & cond_sr_new) | (cond_percent_old & cond_percent_new)

# Improved biostatus (29)
...
sum(cond_same_method & cond_diff_status123_improve) # 29
...
sum(cond_same_method & cond_diff_status123_worsened) # 12
...
sum(cond_3 & cond_6) # --> none

biostatus_merge_diff$explanation[cond_cuid]


#'* data-deficient --> not-assessed (27) *
